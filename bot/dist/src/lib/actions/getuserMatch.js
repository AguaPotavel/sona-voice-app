...
const getUserMatch = (listMatches, listPlayers, discordId) => {
    ...
    const match = listMatches
        ...
        .filter((match) => match.players.some((p) => p.summonerId === player.summonerId))[0];
    if (!match)
        return null;
    return {
        matchId: match.matchId,
        teamOne: match.players.find((p) => p.summonerId === player.summonerId)
            .team === 1,
    };
};
